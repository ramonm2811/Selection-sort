...
function ordenarCartas() {
  ...
  //Ordenar con el Selection Alogrithm
  let min = 0;
  while (min < guardarDatos.length - 1) {
    for (let i = min + 1; i < guardarDatos.length; i++) {
      if (guardarDatos[min].numero > guardarDatos[i].numero) {
        let aux = guardarDatos[min];
        guardarDatos[min] = guardarDatos[i];
        guardarDatos[i] = aux;
